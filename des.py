...
def program2(permutedKey2, plainPart):
	...
	print("K%d" %(x+1),"(after permutation2):", ''.join(permutedKey2))
	print("\nR%d" %x, "after expansion\t:", ''.join(plainPartExp))
	xorResult = toXor(plainPartExp, permutedKey2)
	print("R%d" %x, "after XOR with K%d\t:" %(x+1) ,''.join(xorResult))

	sboxResult = toSbox(xorResult)
	print("R%d" %x, "after SBox\t\t:", ''.join(sboxResult))
	permutedPlain = toPermut(sboxResult, plainP2)
	print("R%d" %x, "after permutation\t:", ''.join(permutedPlain))
	xorResult2 = toXor(permutedPlain, plainPart['left'])
	print("R%d" %(x+1), "= L%d" %x, "XOR f(R%d" %x, ",K%d" %(x+1), ")", ''.join(xorResult2))
	return {'left':originRight, 'right':xorResult2}

plain = input("Enter 8 chars text\t: ")
asciiPlain = toAscii(plain)
binaryPlain = toBinary(asciiPlain)
key = input("Enter 8 chars key\t: ")
asciiKey = toAscii(key)
binaryKey = toBinary(asciiKey)

print("===========================================")
print("\nText in binary\t: ", ''.join(binaryPlain))
print("Key in binary\t: ", ''.join(binaryKey))

permutedPlain = toPermut(binaryPlain, plainIP)

print("\nText after permutation\t:", ''.join(permutedPlain))

plainFirstPart = toLeftRight(permutedPlain)

#print("\nL", ''.join(plainFirstPart['left']))
#print("R", ''.join(plainFirstPart['right']))

permutedKey = toPermut(binaryKey, keyP1)
print("\nKey after permutation\t:", ''.join(permutedKey))
keyPart = toLeftRight(permutedKey)

for x in range(16):
    print("\n================= ROUND", x, "=================")
    print("C%d" %x,"\t:", ''.join(keyPart['left']))
    print("D%d" %x,"\t:", ''.join(keyPart['right']))
    print("L%d" %x,"\t:", ''.join(plainFirstPart['left']))
    print("R%d" %x,"\t:", ''.join(plainFirstPart['right']))
    plainFirstPart = program(plainFirstPart, x)
...
for x in range(len(finalText)):
    ...
    #print(finalText[x], end=' ')
    finalText[x]= chr(int(finalText[x],2))
# print(''.join(finalText))
print(finalText)
print("\n\n============ DECIPHERED TEXT RESULT =============")

newMboh = finalText
binaryNewMboh = toBinary(toAscii(newMboh))
permutedNewMboh = toPermut(binaryNewMboh, plainIP)
newMbohPart = toLeftRight(permutedNewMboh)

for x in reversed(range(16)):
    newMbohPart = program2(K[x], newMbohPart)
finalComb = toCombine(newMbohPart['right'], newMbohPart['left'])
cipherText = toPermut(finalComb, finalPermut)
finalText = toSplit(cipherText,8)
print("\n\n============ CIPHERTEXT RESULT =============")
for x in range(len(finalText)):
